...
-  Give type signatures for everything (see `Type Signatures <#sec-type-signatures>`__.
...
HappyAbsSyn where ``E`` is the Haskell expression from the grammar file (with ``$n`` replaced by ``happy_var_n``).
