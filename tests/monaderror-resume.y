...
%name parseExp Exp
%tokentype { LToken }
%error { abort } { reportError } -- the entire point of this test
...
abort :: Monoid e => [LToken] -> Validate e a -- this would be mzero from MonadPlus
abort _ = V mempty Nothing
...
main = do
  ...
  testStmts exit_code_ref "1+;+1;(+1;1" $ \(_,mb_ast) -> mb_ast == Just ["1 + catch","catch + 1","catch","1"]
